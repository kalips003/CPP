...
void	Bureaucrat::signForm(AForm& form) const {
	try {
		...
		std::cout << C_515 << _name << RESET << " couldn't sign form ("
		<< C_515 << form.getName() << RESET << ") because: " << e.what() << std::endl;
		return;
	}
	std::cout << C_515 << _name << RESET << " signed " << C_515 << form.getName() << RESET << std::endl;
}
